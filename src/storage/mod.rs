...
use std::path::Path;
use rocksdb::{DB, ColumnFamily, Options, ColumnFamilyDescriptor, WriteBatch};
...
impl BlockchainStorage {
    ...
    pub fn get_block(&self, hash: &Hash) -> Result<Option<Block>, Error> {
        ...
        match self.db.get_cf(cfs.blocks, hash)? {
            Some(bytes) => {
                let (block, _): (Block, _) = bincode::decode_from_slice(&bytes, bincode::config::standard())?;
                Ok(Some(block))
            }
            None => Ok(None),
        }
    ...
    pub fn get_block_hash_by_height(&self, height: u64) -> Result<Hash, Error> {
        ...
        match self.db.get_cf(cfs.block_height, height_bytes)? {
            ...
            None => Err(Error::NotFound(format!("Block at height {} not found", height))),
        }
    }
    ...
    pub fn get_latest_height(&self) -> Result<u64, Error> {
        ...
        let mut iter = self.db.iterator_cf(cfs.block_height, rocksdb::IteratorMode::End);
    ...
    pub fn get_transaction(&self, hash: &Hash) -> Result<Option<Transaction>, Error> {
        ...
        match self.db.get_cf(cfs.transactions, hash)? {
            Some(loc_bytes) => {
                let (tx_location, _): (TxLocation, _) = bincode::decode_from_slice(&loc_bytes, bincode::config::standard())?;
                    ...
                    Some(block) => {
                        ...
                        if index < block.transactions.len() {
                            ...
                            Err(Error::Database(format!(
                                "Invalid transaction index {} in block", index
                            )))
                        }
                    }
                    None => Err(Error::NotFound(format!(
                        "Block containing transaction {} not found", hex::encode(hash)
                    ))),
                }
            }
            None => Ok(None),
    ...
    /// Get account state
    pub fn get_account_state(&self, address: &PublicKeyBytes) -> Result<Option<AccountState>, Error> {
        let cfs = self.get_column_families()?;

        match self.db.get_cf(cfs.account_state, address)? {
            Some(bytes) => {
                let (state, _): (AccountState, _) = bincode::decode_from_slice(&bytes, bincode::config::standard())?;
                Ok(Some(state))
            }
            None => Ok(None),
        }
    ...
    pub fn verify_integrity(&self) -> Result<bool, Error> {
        ...
        let mut current_hash = self.get_block_hash_by_height(latest_height)?;
        
        for height in (0..latest_height).rev() {
            let block = self.get_block(&current_hash)?
                .ok_or_else(|| Error::Database(format!("Block with hash {} not found", hex::encode(current_hash))))?;
